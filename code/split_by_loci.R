...
write.csv(promoter_limma_df_train,
          file = file.path(loci_dirs$promoter, "promoter_train.csv"),
          row.names = TRUE)

# Protein coding
protein_coding_limma_df_train <- train_df[, colnames(train_df) %in% rownames(protein_coding_probes)]
write.csv(protein_coding_limma_df_train,
          file = file.path(loci_dirs$protein_coding, "protein_coding_train.csv"),
          row.names = TRUE)

# TSS200
tss200_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$TSS200)]
write.csv(tss200_limma_df_train,
          file = file.path(loci_dirs$tss200, "tss200_train.csv"),
          row.names = TRUE)

# Body
body_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$Body)]
write.csv(body_limma_df_train,
          file = file.path(loci_dirs$body, "body_train.csv"),
          row.names = TRUE)

# stExon
stexon_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$`1stExon`)]
write.csv(stexon_limma_df_train,
          file = file.path(loci_dirs$stExon, "stExon_train.csv"),
          row.names = TRUE)

# 5'UTR
utr5_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$`5'UTR`)]
write.csv(utr5_limma_df_train,
          file = file.path(loci_dirs$utr5, "utr5_train.csv"),
          row.names = TRUE)

# TSS1500
tss1500_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$TSS1500)]
write.csv(tss1500_limma_df_train,
          file = file.path(loci_dirs$tss1500, "tss1500_train.csv"),
          row.names = TRUE)

# 3'UTR
utr3_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$`3'UTR`)]
write.csv(utr3_limma_df_train,
          file = file.path(loci_dirs$utr3, "utr3_train.csv"),
          row.names = TRUE)

# ExonBnd
exonbnd_limma_df_train <- train_df[, colnames(train_df) %in% rownames(lista_subsets$ExonBnd)]
write.csv(exonbnd_limma_df_train,
          file = file.path(loci_dirs$exonbnd, "exonbnd_train.csv"),
          row.names = TRUE)

cat("Division by locis done succesfully!\n")
